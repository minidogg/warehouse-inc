...
Object.entries(game.producers).forEach((producerNameData) => {
  ...
  shopItemElement.q(".buyOptionPrice").textContent =
    `${abbreviateNumber(shop.findCost(data.baseCost, data.costMultiplier, data.owned))} sugar - ${data.sps} SPS - ${data.owned} Owned`;

  shopItemElement.id = shop.compileNum + "_buyOption_" + id;

  shopItemElement.onclick = () => {
    let producerData = game.producers[id];
    let cost = shop.findCost(
      producerData.baseCost,
      producerData.costMultiplier,
      producerData.owned,
    );
    if (cost > game.sugar) {
...
shop.update = () => {
  ...
  for (ee of Object.entries(game.producers)) {
    ...
    if (i < game.currentShopVisible) {
      ...
      el.q(".buyOptionPrice").textContent =
        `${abbreviateNumber(cost)} sugar - ${data.sps} SPS - ${data.owned} Owned`;
    } else {
      ...
      el.q(".buyOptionPrice").textContent =
        `${abbreviateNumber(cost)} sugar - ??? SPS - 0 Owned`;
    }
    el.onmouseover = () => {
