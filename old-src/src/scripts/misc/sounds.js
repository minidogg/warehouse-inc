...
addProperty(game.settingsMd, "soundVolume", {
  type: "slider",
  min: 0,
  max: 100,
});
...
music.musicLoop = () => {
  ...
  document.getElementById("songName").textContent = camelCaseToWords(
    music.current,
  ).replace(/(\.).+/g, "");
...
deepBelow.add("collectSound", () => {
  const randomSound =
    music.soundFiles[Math.floor(Math.random() * music.soundFiles.length)];
